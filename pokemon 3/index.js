...
/**
 * Exibe os detalhes do Pokémon selecionado na área #pokemon-details.
 * Altera a cor de fundo baseado no tipo primário.
 */
function displayPokemonDetails(data) {
    // 1. Identifica o primeiro tipo do Pokémon
    const tipoPrincipal = data.types[0].type.name;

    // 2. Obtém a cor correspondente no mapa (ou cinza padrão se não achar)
    const corFundo = coresTiposPokemon[tipoPrincipal] || '#f0f0f0';

    // 3. Aplica a cor de fundo ao elemento do card
    detailsElement.style.backgroundColor = corFundo;

    // 4. Adiciona a classe CSS para garantir legibilidade do texto
    // (Removemos primeiro para garantir, depois adicionamos)
    detailsElement.classList.add('texto-legivel');

    // 5. Renderiza o HTML interno
    detailsElement.innerHTML = `
        <h2>${data.name.toUpperCase()} (#${data.id})</h2>
        <img src="${data.sprites.front_default}" alt="${data.name}" width="96" height="96">
        <p><strong>Tipo Principal:</strong> ${tipoPrincipal.toUpperCase()}</p>
